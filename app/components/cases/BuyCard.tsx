...
export default function BuyCard({ className = "", item, path = '/' }: Props) {
    return (
        ...
        >
            {/* Added a consistent min-h to the inner content div */}
            <div className="p-3 md:p-4 rounded-2xl bg-[#1C1E2D] text-center relative z-1 group cursor-pointer flex flex-col h-full min-h-[300px] sm:min-h-[320px] lg:min-h-[350px]">
                {/* Background elements */}
                ...
                {/* Case Image */}
                <div className="md:mt-2 mb-4 flex justify-center items-center flex-shrink-0">
                    <img src={item?.img} className='max-h-28 sm:max-h-32 md:max-h-35 lg:max-h-40 mx-auto object-contain transition-all duration-300 group-hover:scale-105' alt="" />
                </div>

                {/* Price and Description */}
                <h4 className='font-black italic text-lg !leading-tight mb-2 flex-shrink-0'>{item?.price}</h4>
                <p className='font-bold text-white text-sm mb-3 md:mb-4 flex-grow'>{item?.des}</p>

                {/* Buy Button */}
                <button className='buy-btn group relative flex items-center justify-center w-full rounded-full min-h-8.5 md:min-h-10 lg:min-h-12 bg-white/10 border border-solid border-white/10 text-sm font-bold text-white mt-auto flex-shrink-0'>
                    {item.btn || 'Buy Now'}
                    <svg className='absolute top-0 left-0 w-full h-full -z-10 opacity-0 invisible transition-all duration-300 group-hover:opacity-100 group-hover:visible' viewBox="0 0 211 48" fill="none" xmlns="http://www.w3.org/2000/svg">
