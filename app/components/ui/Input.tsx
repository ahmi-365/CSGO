...
export default function Input({
    ...
    // Helper to correctly pass the event up to the parent component's handler
    ...
    return (
        <div className={`${className}`}>
            {label && (
                <label
                    // Use the 'id' prop to link the label to the input for accessibility
                    htmlFor={id} 
                    className={`mb-3 block transition-all duration-300 ${
                        input_focus ? "text-white" : "text-[#5D5F70]"
                    } font-inter font-normal !leading-[160%] ${labelClass}`}
                >
                    {label}
                </label>
            )}
            {type === "textarea" ? (
                <textarea
                    ...
                    name={name}
                    id={id}
                    // 🔑 FIX: Pass value and onChange to the native element
                    value={value} 
                ...
                <div className="relative">
                    <input
                        onFocus={() => { set_input_focus(true) }}
                        onBlur={() => { set_input_focus(false) }}
                        type={newType} // Use newType for toggling password visibility
                        name={name}
                        id={id}
                        // 🔑 FIX: Pass value and onChange to the native element
                        value={value} 
                        onChange={handleNativeChange} 
                        className={` px-3 w-full font-inter font-medium text-sm bg-white/8 text-[#BFC0D8]/50 placeholder:text-[#BFC0D8]/50 rounded-xl ${
                            type === "search" ? "pl-11.5" : ""
                        } ${inputClass}`}
                        placeholder={placeholder}
                        required={required}
                    ...
                    {type === "password" && (
                        <button 
                            ...
                            className='absolute top-1/2 -translate-y-1/2 right-0 px-4'
                        >
                            {/* Password Toggle SVG Icons (included for completeness) */}
                            {newType === 'password' ?
                                ...
                                <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <g clipPath="url(#clip0_2120_648)">
                                        <path d="M8 10.0001C9.10457 10.0001 10 9.10469 10 8.0001C10 6.89554 9.10457 6.0001 8 6.0001C6.89543 6.0001 6 6.89554 6 8.0001C6 9.10469 6.89543 10.0001 8 10.0001Z" stroke="white" strokeOpacity={.6} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                                        <path d="M15.3333 8.0001C12.6666 13.3334 3.33329 13.3334 0.666626 8.0001C3.33329 2.66676 12.6666 2.66676 15.3333 8.0001Z" stroke="white" strokeOpacity={.6} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                                    </g>
                                    <defs>
                                        <clipPath id="clip0_2120_648">
                                            <rect width="16" height="16" fill="white" />
                    ...
                    {/* Error Message */}
                    {error && (
                        <span className="text-xs italic font-inter font-normal text-red-400">
                            {error}
                        </span>
                    )}
                </div>
