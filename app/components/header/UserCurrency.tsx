...
// Define a minimal interface for the expected data structure
interface UserData {
    details?: {
        balance: string;
    };
    cc_rate?: string | number;
}
...
export default function UserCurrency({ className = "flex" }: Props) {
    ...
    const [cc_rate, setCcRate] = useState(6.395); // Default value

    // 🚀 useEffect to fetch and set dynamic data from localStorage
    useEffect(() => {
        const fetchUserData = () => {
            ...
            if (storedUserData) {
                try {
                    ...
                    // Set Balance (USD)
                    const balance = parseFloat(parsedData.details?.balance || '0');
                    setWalletAmount(balance);

                    // Set CC Rate
                    let rate = 6.395; // Fallback
                    if (parsedData.cc_rate) {
                        rate = typeof parsedData.cc_rate === 'string' 
                               ? parseFloat(parsedData.cc_rate) 
                               : parsedData.cc_rate;
                    }
                    setCcRate(rate);
                ...
                // Not authenticated or data missing
                setWalletAmount(0);
                setCcRate(6.395);
            }
        };
    ...
    // Calculate CC amount
    const ccAmount = cc_rate !== 0 ? (walletAmount / cc_rate) : 0;
    
    return (
        <div className={`items-center gap-2 ${className}`}>
            {/* Currency Switch Button (USD / CC) */}
            <button 
                onClick={() => setWalletSwitch((prev) => !prev)} 
                className={`relative z-1 min-w-10 min-h-6 gradient-border-two ${walletSwitch ? ' [--bg-color:#4E4270]' : '[--gradient:#FFFFFF29] [--bg-color:#ffffff1a]'} rounded-full`}
            >
                <span className={`absolute transition-all duration-300 ${walletSwitch ? 'left-[calc(100%-20px)]' : 'left-0.5'} top-1/2 -translate-y-1/2 uppercase text-[#171925] text-[10px] size-4.5 flex items-center justify-center bg-white rounded-full font-black`}>
                    {walletSwitch ? 'cc' : '$'}
                </span>
            </button>
            ...
            <button className='flex items-center px-3 justify-center gap-2 rounded-full text-white bg-white/[6%] hover:bg-primary/20 border border-white/[4%] min-w-25 min-h-10'>
                ...
                <span className='block font-bold text-sm text-white min-w-14'>
                    {/* Display logic for hidden vs. visible, USD vs. CC */}
                    {hiddenWallet ?
                        <span className='translate-y-1 block'>{'*'.repeat(String(walletAmount.toFixed(2)).length)}</span>
                        :
                        walletSwitch ?
                            ccAmount.toFixed(3) // CC amount
                            :
                            `$${walletAmount.toFixed(2)}` // USD amount
                    }
                </span>
                
                {/* Toggle Visibility Button */}
                <span className='p-1 -m-1' onClick={(e) => {
                    e.stopPropagation(); // Prevents the outer button's click handler (if any)
                    setHiddenWallet((prev) => !prev);
                }}>
                    {hiddenWallet ?
