...
export default function Header({}: Props) {
  ...
  const fetchWalletBalance = async (authData?: UserData) => {
    ...
    try {
      ...
      const userId =
        userAuthData.user?.id || userAuthData.details?.user_id || 1;
  ...
  const loginInfoLinkStyle =
    "flex items-center gap-3 min-h-11 rounded-xl px-3 font-medium text-white hover:bg-white/10";
  ...
  return (
    <>
      ...
      <header
        className={`flex items-center justify-between gap-2 py-2.5 px-4 lg:px-6 bg-[#1E202C]/10 border-b border-solid border-white/5 z-3 w-full ${
          scrolled ? "sticky top-0 left-0 backdrop-blur-xl" : "relative"
        }`}
      >
        ...
        {!isAuth ? (
          ...
          <div className="flex items-center gap-2 ml-auto lg:ml-0">
            ...
            <Link
              href={"/deposit"}
              className="flex items-center justify-center gap-2 rounded-full bg-white/[6%] size-10 hover:bg-primary/20"
            >
              <Bolt size={20} />
            </Link>
            <div ref={dropdownRef} className="relative z-1">
              <button
                onClick={() => setIsInfoModal((prev) => !prev)}
                className="flex items-center justify-center gap-2 p-2 rounded-full bg-white/[6%] size-10 overflow-hidden relative z-1"
              >
                <img
                  className="absolute top-0 left-0 w-full min-h-full h-auto object-cover"
                  src={avatar}
                  alt={userName}
                />
              </button>
      ...
      <div
        className={`fixed mt-3.5 top-13 right-5 z-10 rounded-2xl bg-[#D7DEFF]/15 border border-solid border-[#D7DEFF]/10 backdrop-blur-[20px] flex flex-col gap-y-1 p-3 w-full max-w-68 transition-all duration-300 ${
          isInfoModal
            ...
            {item.name === "Withdraw" ? (
              <div className="bg-white/4 border border-solid border-white/4 rounded-xl flex items-center gap-2 p-3">
                ...
                <div className="grow flex flex-col gap-y-1.5">
                  <span className="block text-white/60 text-xs font-medium !leading-[120%]">
                    Balance
                  </span>
                  <div className="flex items-center gap-2">
                    ...
                    <button className="grow gradient-border-two min-h-7 text-xs font-satoshi text-white rounded-full flex items-center justify-center uppercase [--bg-color:#4B4172]">
                      <span className="px-2.5 py-1 block size-full">
                        {isLoadingWallet
                          ? "..."
                          : `${(walletBalance / cc_rate).toFixed(3)} CC`}
                      </span>
                    </button>
                  </div>
                </div>
