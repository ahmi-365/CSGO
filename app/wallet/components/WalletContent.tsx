...
// --- Interfaces for our data structures (Exported to be used in other components) ---
...
const initialWalletItems: WalletItem[] = [
  ...
  },
  {
    ...
    icon: (
      ...
      >
        <path
          d="M10.166 6.66667V10L12.2493 12.0833"
          stroke="currentColor"
          strokeWidth="1.5"
          strokeLinecap="round"
...
export default function WalletContent({}: Props) {
  ...
  const [walletItems, setWalletItems] =
    useState<WalletItem[]>(initialWalletItems);
  const [activeWallet, setActiveWallet] = useState<WalletItem>(
    initialWalletItems[0]
  );
  ...
  useEffect(() => {
    const fetchCryptoPrices = async () => {
      ...
      try {
        ...
        if (formattedCryptoList.length > 0) {
          setSelectedCrypto(formattedCryptoList[0]);
        }
      } catch (err: any) {
      ...
      <div className="flex flex-wrap gap-6">
        <div className="flex flex-col gap-y-4 md:gap-y-5 lg:gap-y-6 w-full lg:w-112.5">
          <div className="flex items-center gap-1 p-1 bg-[#1E202C] rounded-2xl">
            {walletItems.map((item, index) => (
              <button
                ...
                className={`grow px-3 md:px-4 min-h-10 lg:min-h-12.5 flex items-center justify-center gap-2 capitalize text-base font-medium rounded-xl hover:text-white ${
          ...
          <div className="bg-white/5 rounded-[20px] p-5">
            <div className="flex items-center justify-between flex-wrap gap-2 mb-3 md:mb-4 lg:mb-5">
              <h4 className="text-lg md:text-xl xl:text-[22px] !leading-[130%]">
              ...
              <div className="flex items-center gap-1">
                ...
                <p className="text-base text-[#6F7083] font-satoshi font-medium !leading-[100%]">
                  Live
                </p>
              </div>
            </div>
            <div className="overflow-y-auto max-h-155 -mr-3 pr-3">
              {loading && (
                <p className="text-center p-4 text-[#6F7083]">
                  Loading prices...
                </p>
              )}
              {error && (
              ...
              {!loading &&
                !error &&
                activeWallet?.crypto?.map((item, index) => (
                  <div
                    className={`flex items-center justify-between p-3 md:p-4 rounded-xl border border-solid cursor-pointer transition-all duration-300 mb-2 ${
                      selectedCrypto?.currency === item.currency
                        ? "border-[#F7931A]/50 bg-[#F7931A]/20"
                        : "border-transparent bg-white/5 hover:border-[#F7931A]/10 hover:bg-[#F7931A]/20"
                    }`}
                    key={index}
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      />
                      <div>
                        <h3 className="text-sm md:text-base xl:text-lg text-white font-satoshi font-bold !leading-[120%] mb-2">
                          {item.name}
                        </h3>
                        <h4 className="text-xs text-[#6F7083] font-satoshi font-medium !leading-[100%]">
                          {item.currency}
                        </h4>
                      </div>
                    </div>
        ...
        <div className="w-full lg:w-[calc(100%-475px)]">
          <div className="h-full rounded-[20px] bg-white/[6%] border border-solid border-white/15 w-full p-4 md:p-6">
            {activeWallet?.name?.toLowerCase() === "deposit" && (
              <Deposit selectedCrypto={selectedCrypto} />
            )}
            {activeWallet?.name?.toLowerCase() === "withdraw" && (
              <Withdraw selectedCrypto={selectedCrypto} />
            )}
