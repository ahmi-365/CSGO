...
export default function WalletContent({ }: Props) {
    ...
    const walletItems: WalletItem[] = [
        {
            ...
            crypto: [
                {
                    icon: "/img/wallet/1.svg",
                    name: 'Bitcoin',
                    currency: 'BTC',
                    value: '118,169.00',
                    share_percent: '1.89.00%',
                },
                {
                    icon: "/img/wallet/2.svg",
                    name: 'Binance',
                    currency: 'BUSD',
                    value: '118,169.00',
                    share_percent: '1.89.00%',
    ...
    const [activeWallet, setActiveWallet] = useState(walletItems[0]);

    useEffect(() => {
        if (currentType) {
            const found = walletItems.find(item => item.name?.toLowerCase() === currentType.toLowerCase()) || walletItems[0];
                            ...
                            {activeWallet?.crypto?.map((item, index) => (
                                <div className="flex items-center justify-between p-3 md:p-4 bg-white/4 rounded-xl border border-solid border-transparent cursor-pointer hover:border-[#F7931A]/10 mb-2 hover:bg-[#F7931A]/20 transition-all duration-400" key={index}>
                                    <div className="flex items-center gap-3">
                                        <img src={item.icon} className='size-10 rounded-full object-cover' alt="" />
                                        <div className="">
